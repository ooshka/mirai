...
- `GET /mcp/index/query?q=<text>&limit=<n>&path_prefix=<relative/path>`
  ...
  - `snippet_offset` is additive metadata for grounding hints. It is `null` when no lexical token overlap is found in a returned chunk (for example, semantic hit with non-overlapping text).
