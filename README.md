...
- `POST /mcp/workflow/plan`
  ...
  - Canonical draft handoff action: `{"action":"workflow.draft_patch","reason":"...","params":{"instruction":"...","path":"notes/today.md","context":{...optional object...},"workflow_action_id":"workflow-action-2-..."}}`. When `profile` is supplied on the plan request, the same selector is included in the generated `workflow.draft_patch` params for thin-client handoff.
  - Response: `{ "intent": "...", "provider": "openai|local", "rationale": "...", "actions": [{"action":"notes.read","reason":"...","params":{"path":"notes/today.md"}},{"action":"workflow.draft_patch","reason":"...","params":{"instruction":"...","path":"notes/today.md","context":{"source":"planner"},"workflow_action_id":"workflow-action-2-..."}}] }`
  ...
  - Correlation note: `workflow_action_id` is generated by `mirai` for planner-returned draft actions so clients can pair later dry-run, apply, or execute results with the originating planned action. It is not an authorization token, persistence key, or workflow session ID.
...
- `POST /mcp/workflow/draft_patch`
  ...
  - Response: `{ "edit_intent": { "path": "notes/today.md", "operation": "replace_content", "content": "# Today\n..." }, "trace": { "provider": "openai", "model": "gpt-4.1-mini", "workflow_action_id": "workflow-action-2-...", "target": { "path": "notes/today.md", "content_bytes": 12 }, "context": { ... }, "validation": { "status": "valid", "path": "notes/today.md", "hunk_count": 1, "net_line_delta": 3 }, "apply_ready": true, "audit": { "patch": "--- a/notes/today.md\n+++ b/notes/today.md\n..." } } }`
  ...
  - Safety note: this endpoint validates draft shape and server-owned patch translation but does not apply/commit changes. Use `trace.audit.patch`, `trace.validation`, `trace.apply_ready`, and optional `trace.workflow_action_id` to inspect the dry run before calling apply or execute.
...
- `POST /mcp/workflow/execute`
  ...
  - Request: `{ "action": "workflow.draft_patch", "params": { "instruction": "add today's summary", "path": "notes/today.md", "context": { ...optional object... }, "profile": "hosted|local|auto", "workflow_action_id": "workflow-action-2-..." } }`
  - Response: `{ "action": "workflow.draft_patch", "path": "notes/today.md", "hunk_count": 1, "net_line_delta": 1, "audit": { "patch": "--- a/notes/today.md\n+++ b/notes/today.md\n...", "provider": "openai", "model": "gpt-4.1-mini", "workflow_action_id": "workflow-action-2-..." } }`
  - Contract note: this endpoint consumes the same canonical `workflow.draft_patch` action envelope returned by planning and accepted by `/mcp/workflow/draft_patch` and `/mcp/workflow/apply_patch`; the response echoes that canonical action at top level, optional planner correlation metadata in `audit.workflow_action_id`, and invalid request fields are still reported as `invalid_workflow_execute`.
