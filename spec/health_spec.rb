...
RSpec.describe "Health" do
  ...
  it "returns config with policy and retrieval diagnostics" do
    ...
    expect(body.fetch("mcp_semantic_configured")).to eq(App.settings.mcp_semantic_configured)
    expect(body.fetch("mcp_semantic_ingestion_enabled")).to eq(App.settings.mcp_semantic_ingestion_enabled)
    ...
    expect(body.fetch("mcp_local_semantic_base_url")).to eq(App.settings.mcp_local_semantic_base_url)
