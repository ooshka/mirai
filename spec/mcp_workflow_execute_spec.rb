...
RSpec.describe "MCP workflow execute endpoint" do
  ...
  it "executes the canonical planner action through one server-owned endpoint" do
  ...
  it "uses an explicit local workflow profile for execute selection" do
    FileUtils.mkdir_p(File.join(@notes_root, "notes"))
    file_path = File.join(@notes_root, "notes/today.md")
    File.write(file_path, "alpha\n")
    git!("add", "notes/today.md")
  ...
  it "resolves workflow profile once for execute requests" do
    ...
    expect(last_response.status).to eq(200)
    expect(JSON.parse(last_response.body).fetch("action")).to eq("workflow.draft_patch")
    expect(File.read(file_path)).to eq("alpha\nbeta\n")
  end
  ...
  it "returns invalid_workflow_execute when workflow_action_id is not a string" do
    post "/mcp/workflow/execute", JSON.generate(
      {
        action: "workflow.draft_patch",
        params: {
  ...
  it "returns policy_denied for workflow execute in read_only policy mode" do
